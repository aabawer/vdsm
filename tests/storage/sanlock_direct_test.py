...
from vdsm.common.constants import EXT_SANLOCK
...
class FakeSanlockDirectDump(object):
...
@pytest.fixture
def fake_run_dump(monkeypatch):
    monkeypatch.setattr(sanlock_direct, "run_dump", FakeSanlockDirectDump())
...
def _sanlock_direct(cmd, args, block_size, alignment):
    ...
    with utils.stopwatch("sanlock direct {}".format(args)):
        commands.run([EXT_SANLOCK, "direct", cmd] + args + options)
